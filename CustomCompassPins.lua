...
function lib:AddCustomPin(pinType, pinCallback, layout, savedVarTable)
  ...
  self.pinManager:SetCustomPinEnabled(pinType, enabled)
end
...
function CompassPinManager:Initialize2(...)
  self.pinData = {}
  self.defaultAngle = 1
  self.customPinEnabled = {} -- internal compass pin visibility state
end
...
function CompassPinManager:SetCustomPinEnabled(pinType, state)
  if type(state) == "boolean" then
    self.customPinEnabled[pinType] = state
  end
end

function lib:SetCustomPinEnabled(...)
  return self.pinManager:SetCustomPinEnabled(...)
end

function CompassPinManager:IsCustomPinEnabled(pinType)
  local state = self.customPinEnabled[pinType]
  return state ~= false -- Treat nil as true (visible by default)
end

function lib:IsCustomPinEnabled(...)
  return self.pinManager:IsCustomPinEnabled(...)
end
...
function CompassPinManager:CreatePin(pinType, pinTag, xLoc, yLoc, pinName, ...)
  if not self:IsCustomPinEnabled(pinType) then return end
